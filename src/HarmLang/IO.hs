...
instance MIDIable NoteProgression where
    ...
    makeTrack ((Note (Pitch (PitchClass p) (Octave o)) (Time t)):rest) = (0, TempoChange tempo):(let
        (n, d) = (numerator t, denominator t)
        num = basenote + (12 * o) + p
        start = 0
...
instance MIDIable TimedChordProgression where
    makeTrack [] = [(0,  TrackEnd)]
    makeTrack ((TimedChord (Harmony (PitchClass p) intervals) (Time t)):rest) = (0, TempoChange tempo):(let
        (n, d) = (numerator t, denominator t)
